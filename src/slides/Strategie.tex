...
\begin{frame}{Markov-Kette}
    \begin{itemize}
        ...
        \item Genauer: IPD ist Markov-Prozess $(X_r)_{r\in \mathbb{N}^+_{< n}}$ der Länge $n \in \mathbb{N}^+$, es gilt Markov-Eigenschaft \parencite{markov-1906, sandmann-2007}:
        \[
        ...
        \item Daher: Markov-Kette ist homogen \parencite{press-dyson-2012}: $ \forall r, r\prime \in \mathbb{N}^+_{< n}, \. \forall z_1, z_2 \in Z: P\bigl( X_{r + 1} = z_2 \,\big|\, X_r = z_1 \bigr) = P\bigl( X_{r\prime + 1} = z_2 \,\big|\, X_{r\prime} = z_1 \bigr)$
    \end{itemize}
\end{frame}

\begin{frame}{Memory-One ist gleich Memory-Many}
    \begin{itemize}
        \item Behauptung: Zu Jeder Memory-N Strategie existiert eine äquivalente Memory-1 Strategie
...
\begin{frame}{Zero-Determinant Strategies I}
    \begin{itemize}
        ...
        \item Beweis: LGS für $ZD$ lässt sich aus Markov-Matrix ableiten (Idee Tafel)
    \end{itemize}
\end{frame}
...
\begin{frame}{Zero-Determinant Strategies II}
    \begin{itemize}
        ...
        \item Forcierung $F$ mit \\
        $\alpha = 0 \neq \beta \rightarrow \text{score}_{g_n}(s^{p_2}) = \frac{-\gamma}{\beta}$
        \begin{itemize}
            \item Gegner-Score ist forciert: $P \leq \text{score}(p_2) \leq R$
            \item Aber: Eigener Score unbeeinflusst
        \end{itemize}
        \item Erpressung $E$ mit
